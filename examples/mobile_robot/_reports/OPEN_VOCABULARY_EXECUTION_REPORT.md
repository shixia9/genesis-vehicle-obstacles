...
已完成开放词汇视觉的第一阶段工程骨架和真实车载图像基准，但尚未达到“自然语言目标可直接驱动导航”的准入条件。

当前结论：

- YOLO-World 可以在车载 RGB 帧上按文本提示运行，且输出多个候选框；
- 当前 Genesis 合成图像上的 zero-shot 结果不稳定，不能把候选直接交给路径规划；
- OWLv2 已提供本地离线适配器，但本次 Hugging Face 权重下载因连接超时未完成，因此没有伪造第二模型的评测结论；
...
- `resolve_open_vocab_device("auto")` 遵循 MPS 优先、CPU 回退；当前机器记录为 `auto_cpu`；
- MPS 算子运行时失败时自动回退 CPU；
- 模型路径必须是本地文件，推理阶段不隐式下载。

新增 [owlv2_grounder.py](../vision/owlv2_grounder.py)：

- `Owlv2Grounder` 可选后端；
- 只接受包含 `save_pretrained` 文件的本地目录；
- 使用 `local_files_only=True`，缺文件时明确报错；
- 与 YOLO-World 使用同一候选输出协议。

### 2.2 离线评估入口

新增 [evaluate_open_vocab.py](../vision/evaluate_open_vocab.py)：

- 支持 `yolo-world` 和 `owlv2` 后端；
...
已通过受控网络安装可选 `transformers`/`safetensors` 依赖，并尝试下载 `google/owlv2-base-patch16-ensemble` 到 `weights/open_vocab/owlv2-base-patch16-ensemble/`。本次访问 `huggingface.co` 多次连接超时，未获得 processor 或模型文件；目录未作为可用权重使用。
...
out = "weights/open_vocab/owlv2-base-patch16-ensemble"
Owlv2Processor.from_pretrained(model_id).save_pretrained(out)
Owlv2ForObjectDetection.from_pretrained(model_id).save_pretrained(out, safe_serialization=True)
PY
```

下载后必须先用同一 45 帧和 OOD 数据集评估，再决定主模型；不能因为模型名称更强就直接替换 YOLO-World。
...
1. 在可联网环境完成 OWLv2 下载并做同口径对比；
2. 生成真正的 Genesis OOD 测试集（绿色柱子、平台、新材质、多目标、目标缺失、诱饵物体），按物体资产/语义组合划分，不能按相邻帧随机拆分；
3. 补齐 Phrase Grounding Recall、Top-1、缺失目标误报率、歧义检出率和 RGB-D 三维误差；
4. 增加 mask/ROI 属性验证、多帧确认和 `NO_VISUAL_MATCH`/`AMBIGUOUS_TARGET` 状态机；
5. 通过冻结指标后，才接入主动搜索、目标附近姿态和路径规划；
6. LLM 仍保持独立：只输出完整开放指代表达和约束，不输出坐标、路径或轮速。

在第 2～4 项未通过前，任何“行驶到黄色小车/绿色柱子/平台附近”的演示都只能作为候选可视化，不应宣称已经实现自然语言导航。
