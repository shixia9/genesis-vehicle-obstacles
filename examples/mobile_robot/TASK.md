...
- [X] `[P0]` 确定沿途需要识别的类别集合：`car/box_obstacle/cylinder_obstacle`。
- [X] `[P0]` 确定先使用运动学小车验收，URDF 后续单独校准。
- [X] `[P0]` 固定本地随机初始化 YOLO 权重、CPU 推理和 SHA-256；正式交付前仍需确认 Ultralytics AGPL-3.0 许可证。
- [X] 固化现有简易路线、目的地、随机种子和运行命令。
- [X] 记录无视觉推理时的到达步数、碰撞、实时率和传感器输出。

**交付物：** 场景/类别清单、基线 `summary.json` 和演示截图。

### VC1：沿途多物体场景

- [X] `[P0]` 新增 `vision_route_showcase` 场景。
- [X] `[P0]` 沿路线布置至少 3 类物体，覆盖近/远、左右和部分遮挡视角。
- [X] `[P0]` 保证物体不使预设路线变成不可达。
- [X] 为物体增加稳定 ID、类别、颜色、footprint 和 entity ID 注册。
- [X] 支持固定种子和可选位置随机化。
- [X] 输出运行日志、标注图和语义对象清单；俯视图仍可作为展示增强项。

**验收：** 不启用视觉时，小车仍能按简易策略无碰到达；整段路线中所有验收物体至少进入一次相机有效视野。

### VC2：离线模型可行性

- [X] `[P0]` 保存一轮完整路线的车载 RGB 数据。
- [X] `[P0]` 使用本地 YOLO 评估器对独立 test 图片推理。
- [X] 统计每类物体在不同距离、角度和遮挡条件下的检测效果；当前 box/cylinder recall 未达 90%。
- [X] 判断随机初始化自定义模型可行性；不使用外部预训练权重。
- [X] 使用 Genesis segmentation 生成 bbox 标注（当前先采用 YOLO Detection，不先做 YOLO-Seg）。
- [X] 训练自定义 YOLO 权重；仍需扩展姿态/遮挡/光照数据以达到最终门槛。
- [X] 固定模型版本、类别映射、置信度和输入尺寸。

**验收：** 在冻结验证集上达到第 13 节门槛，标注图经人工抽查类别和框位置正确。

### VC3：实时“边行驶边识别”闭环

- [X] `[P0]` 新增 `room_navigation_vision.py`，不修改稳定基线行为。
...
- [X] 标定黄色、红色、蓝色等 RGB 色差区间，并覆盖 Genesis 暗黄色渲染。
- [X] bbox 模式去除边界和背景；后续可进一步接入 segmentation mask。
- [X] 统一 RGB-D 相机模型，完成 Genesis 已知内外参和 RGB 到 Depth 的精确重投影。
- [X] 输出检测目标表面距离和距离置信度。
- [X] 输出相机坐标、机器人坐标和世界坐标。
- [ ] 与 LiDAR 投影/最近回波交叉检查明显异常距离。
- [X] 实现多帧关联和稳定 `track_id`；轨迹确认/过期策略仍需增强。
- [X] 在行程结束后生成 `tracked_objects.json` 沿途物体清单。

**验收：** 同一物体跨帧 ID 基本稳定，颜色和距离达到冻结指标，旧结果不会被误记为当前帧结果。

### VC5：控制、安全与性能

- [X] `[P0]` 回归 waypoint + LiDAR 安全层，视觉开/关均能稳定运行。
- [X] `[P0]` 确保正常到达和碰撞终止都会输出零速度；用户中断仍需单独补测。
- [X] 添加视觉延迟统计，异常检测结果不接管控制循环。
- [ ] 推理影响实时率时切换到 `maxsize=1` 的 latest-only 异步模式。
- [X] 记录推理延迟 P50/P95/max、结果年龄和异常数；仿真 FPS 仍需统一工具统计。
- [X] 已覆盖模型缺失、空检测和推理异常的安全降级；慢推理注入测试仍待补充。
